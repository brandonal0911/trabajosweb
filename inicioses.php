...
if(isset($_COOKIE["recuerdame"])){//Si marco recuerdame entonces se hara lo q sigue
	...
	if(isset($_COOKIE["usuario"]) && isset($_COOKIE["tiempo_inicio"])){// Verifica si hay cookies guardadas
		...
		if($tiempo_transcurrido <= 120){// Aqui es una condicion para saber si aún no han pasado 120 segundos
			...
			// Si no han pasado los 120 segundos resstauramos la sesión desde las cookies
			...
			setcookie("tiempo_inicio", "", time() -3600);
			setcookie("recuerdame", "", time() -3600);
			//se pone las comillas solas porque no estamos guardando un valor, y el -3600 porque asi la cookie se destruye casi al instante
			session_destroy();
...
//Verificamos si hay un inicio de sesion si no redirigimos al login
if(isset($_SESSION["usuario"])){//preguntas si hay una sesion 
     // Si hay sesion no hace nada sigue adelante
}else{
