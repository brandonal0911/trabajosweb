...
if(isset($_COOKIE["recuerdame"])){ // Aqui se hace una condicion y pregunta si existe la cookie de recuerdame
	...
	if(isset($_COOKIE["usuario"]) && isset($_COOKIE["tiempo_inicio"])){
		...
		if($tiempo_transcurrido <= 120){
			...
			setcookie("usuario", "", time() -3600); // Borra cookie de usuario
			setcookie("tiempo_inicio", "", time() -3600); // Borra cookie de tiempo
			setcookie("recuerdame", "", time() -3600); // Borra cookie de recuérdame
		}
	}
}
...
// Si marco recuerdame no hace nada
if(isset($_COOKIE["recuerdame"])){
    // No hace nada
}else{
    // Si no marco recuerdame y hay sesión
